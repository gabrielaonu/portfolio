...
function semplice_post_row($post_id, $post_type, $post_title, $post_status, $is_duplicate, $thumbnail, $is_semplice, $slug, $projects_view) {
	...
	if($post_type == 'project' && $projects_view == 'thumb') {
		...
		$output .= '
			<div class="project admin-column' . $is_semplice_class . '" data-xl-width="3" id="' . $post_id . '" data-thumbnail-src="' . $thumbnail['src'] . '" ' . $css . '>
				' . $activate_semplice . '
	...
	if(null !== $posts) {
		...
		foreach ($posts as $post) {
			$projects[$i] = array(
				...
				'pp_thumbnail' => semplice_get_thumbnail($post->ID, 'project_panel'),
				'nextprev_thumbnail' => semplice_get_thumbnail($post->ID, 'nextprev'),
