...
function semplice_about() {
	...
	if($license['is_valid'] && isset($licenses[$license['product']])) {
		...
		<div id="studio-feature-' . $feature . '" class="popup studio-popup">
			...
			<div class="popup-inner feature-popup-inner">
				...
				<div class="popup-footer">
					' . $see_it_live . '
					<a href="https://transactions.sendowl.com/products/583885/B3B280C1/add_to_cart" class="feature-purchase">Upgrade to Studio for $39</a>
				</div>					
			</div>
		</div>
	';
...
function semplice_get_module_svgs() {
	...
	// modules
	$modules = array('paragraph', 'text', 'image', 'gallery', 'video', 'spacer', 'button', 'oembed', 'portfoliogrid', 'advancedportfoliogrid', 'code', 'share', 'socialprofiles', 'dribbble', 'instagram', 'gallerygrid', 'mailchimp', 'beforeafter');
...
function semplice_inter_webfont($wrapped) {
	...
	$inter = '';
	$weights = array('Regular' => '400', 'Bold' => '700');

	// iterate weights
	foreach ($weights as $weight => $weight_num) {
		$inter .= '
			@font-face {
				font-family: "Inter";
				font-style:  normal;
				...
				font-display: swap;
				src: url("' . get_template_directory_uri() . '/assets/fonts/Inter-' . $weight . '.woff2?v=3.9") format("woff2"),
					url("' . get_template_directory_uri() . '/assets/fonts/Inter-' . $weight . '.woff?v=3.9") format("woff");
				}
		';
	}
